...
export default class Boss {
    ...
    if (target) {
      ...
      if (distance > this.attackRange + target.radius + 8) {
        ...
        if (ready) {
          ...
          if (onHit) {
            onHit({ x: target.position.x, y: target.position.y, radius: this.attackRange * 0.9, duration: 0.35 });
          }
        }
      }
    } else {
      // wander slowly if no target
  ...
  findClosestCreature(creatures) {
    ...
    for (const creature of creatures) {
      ...
      const dist = Math.hypot(creature.position.x - this.position.x, creature.position.y - this.position.y);
      if (dist < minDist) {
        minDist = dist;
        closest = creature;
      }
